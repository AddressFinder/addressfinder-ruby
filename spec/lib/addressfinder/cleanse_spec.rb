...
RSpec.describe AddressFinder::Cleanse do
  ...
  describe '#build_request' do
    ...
    context 'with a key and secret override' do
      let(:args){ {q: '186 willis st', key: 'AAA', secret: 'BBB', http: http} }

      it { expect(request_uri).to eq('/api/nz/address/cleanse?q=186%20willis%20st&format=json&key=AAA&secret=BBB') }
    end
